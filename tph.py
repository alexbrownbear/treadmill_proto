...
if True:
    ...
    print('>>> send first check code')
    tph.send_first_check_code()
    tph.agreement_status = 2
    print('>>> start manage data')
    tph.manage_data(np.array(bytearray.fromhex('a90804df716e8eeb'), dtype=np.int8))
    print('>>> end manage data')
    # print('notify:%s, rsp-random: %s' % ("a90804df716e8eeb", tph.rsp_random_num % 6))

    exit()
    tph = TreadmillProtocolHelper()
    tph.send_first_check_code()
    tph.agreement_status = 2
    tph.manage_data(np.array(bytearray.fromhex('a90804baffbb451e'), dtype=np.int8))
    print('notify:%s, rsp-random: %s' % ("a90804baffbb451e", tph.rsp_random_num % 6))


    # print(send_single_code(1, 100).tobytes().hex())
    # $send_first_check_code()
    # print(send_single_code(np.int8(1), np.int8(10)).tobytes())
    # print(send_single_code(-93, 1).tobytes())
